...
static inline void display_route_frame(display_data_t *dd, uint32_t id, const uint8_t data[8])
{
    switch (id)
    {
    case CAN_ID_BPS_SAFETY_STATE:
        display_decode_bps_safety(&dd->bps_safety, data);
        dd->update_flags |= DFLAG_BPS_SAFETY;
        break;
    case CAN_ID_BPS_POWER_LIMIT:
        display_decode_bps_power(&dd->bps_power, data);
        dd->update_flags |= DFLAG_BPS_POWER;
        break;
    case CAN_ID_VEGA_DRIVER_INPUTS:
        display_decode_vega_inputs(&dd->vega_inputs, data);
        dd->update_flags |= DFLAG_VEGA_INPUTS;
        break;
    case CAN_ID_VEGA_VEHICLE_STATUS:
        display_decode_vega_status(&dd->vega_status, data);
        dd->update_flags |= DFLAG_VEGA_STATUS;
        break;
    case CAN_ID_MC_STATUS1:
        display_decode_mc_status1(&dd->mc_status1, data);
        dd->update_flags |= DFLAG_MC_STATUS1;
        break;
    case CAN_ID_MC_STATUS2:
        display_decode_mc_status2(&dd->mc_status2, data);
        dd->update_flags |= DFLAG_MC_STATUS2;
        break;
    case CAN_ID_R_BMS_CELL_EXTREMES:
        display_decode_cell_extremes(&dd->cells, data);
        dd->update_flags |= DFLAG_CELLS;
        break;
    case CAN_ID_R_BMS_TEMP_EXTREMES:
        display_decode_temp_extremes(&dd->temps, data);
        dd->update_flags |= DFLAG_TEMPS;
        break;
    case CAN_ID_BPS_SUPPL_BATTERY:
        display_decode_suppl_batt(&dd->suppl, data);
        dd->update_flags |= DFLAG_SUPPL;
        break;
    case CAN_ID_MPPT1_POWER_MEAS:
        display_decode_mppt_power(&dd->mppt[0], data);
        dd->update_flags |= DFLAG_MPPT1;
        break;
    case CAN_ID_MPPT2_POWER_MEAS:
        display_decode_mppt_power(&dd->mppt[1], data);
        dd->update_flags |= DFLAG_MPPT2;
        break;
    case CAN_ID_MPPT3_POWER_MEAS:
        display_decode_mppt_power(&dd->mppt[2], data);
        dd->update_flags |= DFLAG_MPPT3;
        break;
    case CAN_ID_MPPT4_POWER_MEAS:
        display_decode_mppt_power(&dd->mppt[3], data);
        dd->update_flags |= DFLAG_MPPT4;
        break;
    case CAN_ID_ALTAIR_GPS_POS:
        display_decode_gps_pos(&dd->gps_pos, data);
        dd->update_flags |= DFLAG_GPS_POS;
        break;
    case CAN_ID_ALTAIR_GPS_NAV:
        display_decode_gps_nav(&dd->gps_nav, data);
        dd->update_flags |= DFLAG_GPS_NAV;
        break;
    case CAN_ID_ALTAIR_GPS_TIME:
        display_decode_gps_time(&dd->gps_time, data);
        dd->update_flags |= DFLAG_GPS_TIME;
        break;
    case CAN_ID_BPS_CHARGING_INFO:
        display_decode_charging(&dd->charging, data);
        dd->update_flags |= DFLAG_CHARGING;
        break;
    case CAN_ID_VEGA_SAFETY_STATE:
        display_decode_vega_safety(&dd->vega_safety, data);
        dd->update_flags |= DFLAG_VEGA_SAFETY;
        break;
    case CAN_ID_R_BMS_FAULT:
        display_decode_bms_fault(&dd->bms_fault, data);
        dd->update_flags |= DFLAG_BMS_FAULT;
        break;
    case CAN_ID_R_BMS_ENERGY:
        display_decode_bms_energy(&dd->bms_energy, data);
        dd->update_flags |= DFLAG_BMS_ENERGY;
        break;
    case CAN_ID_R_BMS_ADAPT_ENERGY:
        display_decode_bms_adapt_energy(&dd->bms_adapt, data);
        dd->update_flags |= DFLAG_BMS_ADAPT_ENERGY;
        break;
    }
}
