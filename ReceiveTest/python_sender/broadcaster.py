...
SCENARIOS = [
    {
        ...
        "speed": 47.0,  "throttle_pct": 0.35, "regen_pct": 0.0,
        "pack_v": 97.2, "pack_i": 18.5,        "soc": 72.0,
        "hi_cell_v": 3.612, "lo_cell_v": 3.589,
        "hi_temp": 34,  "lo_temp": 28,
        "solar_w": [218, 215, 211, 198],
        "fault_code": 0, "safety_state": 0, "grace_timer": 0, "derate_pct": 0,
        "warning": False, "charging": False,
    },
    {
        "name": "Acceleration",
        "speed": 72.0,  "throttle_pct": 0.85, "regen_pct": 0.0,
        "pack_v": 94.1, "pack_i": 45.2,        "soc": 68.0,
        "hi_cell_v": 3.520, "lo_cell_v": 3.485,
    ...
    {
        "name": "Regen Braking",
        "speed": 35.0,  "throttle_pct": 0.0,  "regen_pct": 0.65,
        "pack_v": 99.8, "pack_i": -8.5,        "soc": 74.0,
        "hi_cell_v": 3.700, "lo_cell_v": 3.680,
    ...
    },
    {
        ...
        "speed": 30.0,  "throttle_pct": 0.20, "regen_pct": 0.0,
        "pack_v": 82.4, "pack_i": 12.0,        "soc": 15.0,
        "hi_cell_v": 3.120, "lo_cell_v": 3.020,
        "hi_temp": 41,  "lo_temp": 33,
        "solar_w": [105, 98, 102, 95],
        "fault_code": 0, "safety_state": 0, "grace_timer": 0, "derate_pct": 0,
        "warning": True, "charging": False,
    },
    {
        "name": "FAULT - Precharge",
        "speed": 25.0,  "throttle_pct": 0.0,  "regen_pct": 0.0,
        "pack_v": 96.0, "pack_i": 2.0,         "soc": 65.0,
        "hi_cell_v": 3.580, "lo_cell_v": 3.550,
    ...
    {
        "name": "Charging",
        "speed": 0.0,    "throttle_pct": 0.0,  "regen_pct": 0.0,
        "pack_v": 108.5, "pack_i": -12.0,       "soc": 88.0,
        "hi_cell_v": 4.020, "lo_cell_v": 4.005,
        "hi_temp": 32,  "lo_temp": 27,
        "solar_w": [0, 0, 0, 0],
        "fault_code": 0, "safety_state": 0, "grace_timer": 0, "derate_pct": 0,
        "warning": False, "charging": True,
    },
]
...
def enc_bps_safety(vstate, sstate, hv_rdy, hazard, fault,
                   red, yellow, dcdc, batt,
                   grace, relays0, relays1,
                   prech_done, vcs_mia, bms_mia, derate_pct):
    d = bytearray(8)
...
def enc_vega_inputs(pedal, brake, thr_out, reg_out, brk_sw, thr_sw, mode, regen):
    d = bytearray(8)
    d[0] = pedal & 0xFF;  d[1] = (pedal >> 8) & 0xFF
    d[2] = brake & 0xFF;  d[3] = (brake >> 8) & 0xFF
    d[4] = _u8(thr_out)
    d[5] = _u8(reg_out)
...
def enc_mc_status1(rpm, motor_a, batt_v, err):
    ...
    d = bytearray(8)
    d[0] = rpm & 0xFF;  d[1] = (rpm >> 8) & 0xFF
    d[2] = mi & 0xFF;   d[3] = (mi  >> 8) & 0xFF
    d[4] = bv & 0xFF;   d[5] = (bv  >> 8) & 0xFF
    d[6] = err & 0xFF;  d[7] = (err >> 8) & 0xFF
    return d
...
def enc_cells(hi_v, hi_id, lo_v, lo_id):
    ...
    d = bytearray(8)
    d[0] = hv & 0xFF;  d[1] = (hv >> 8) & 0xFF
    d[2] = hi_id & 0xFF
    d[3] = lv & 0xFF;  d[4] = (lv >> 8) & 0xFF
    d[5] = lo_id & 0xFF
    d[6] = 0;          d[7] = 0
    return d
...
def enc_temps(hi, hi_id, lo, lo_id, avg, board):
    d = bytearray(8)
    d[0] = hi & 0xFF;    d[1] = hi_id & 0xFF
    d[2] = lo & 0xFF;    d[3] = lo_id & 0xFF
    d[4] = avg & 0xFF;   d[5] = board & 0xFF
    d[6] = 0;            d[7] = 0
    return d
...
def enc_gps_nav(speed_ms, heading, alt_m, fix, sats):
    ...
    d = bytearray(8)
    d[0] = sp & 0xFF;  d[1] = (sp >> 8) & 0xFF
    d[2] = hd & 0xFF;  d[3] = (hd >> 8) & 0xFF
    d[4:6] = _s16(alt_m / 0.1)
    d[6] = fix & 0xFF; d[7] = sats & 0xFF
    return d
...
def enc_charging(active, connected, out_v, out_a, temp):
    ...
    d[1] = cv & 0xFF;  d[2] = (cv >> 8) & 0xFF
    d[3:5] = _s16((out_a + 320.0) / 0.1)
    d[5] = _u8(temp + 40)
    d[6] = 0;  d[7] = _next_alive()
    return d
...
def broadcast_100ms(sc, elapsed_s):
    ...
    reason = 0
    if sc["warning"]:  reason |= 0x10
    if derate > 0:     reason |= 0x40
    if max_reg == 0:   reason |= 0x20

    send_frame(ID_BPS_SAFETY, enc_bps_safety(
        vstate, sc["safety_state"], True,
        sc["fault_code"] != 0, sc["fault_code"],
        sc["fault_code"] != 0, sc["warning"],
        True, True,
        grace, relays0, relays1,
    ...
    send_frame(ID_MC_STATUS1, enc_mc_status1(
        rpm, sc["pack_i"] * 0.8 + wobble, sc["pack_v"] + wobble, 0
    ))

    send_frame(ID_MC_STATUS2, enc_mc_status2(
        thr_out,
        int(sc["hi_temp"] + 8  + wobble),
        int(sc["hi_temp"] + 17 + wobble)
    ))
...
def broadcast_200ms(sc, elapsed_s):
    ...
    send_frame(ID_VEGA_STATUS, enc_vega_status(
        ...
        1 if sc["speed"] > 0 else 0,
        sc["regen_pct"] > 0, True, 0
    ))

    send_frame(ID_CELL_EXTR, enc_cells(
        sc["hi_cell_v"] + wobble * 0.002, 14,
        sc["lo_cell_v"] + wobble * 0.001, 3
    ))
